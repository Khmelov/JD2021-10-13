...
public class Parser {
    ...
    Var calc(String expression) throws CalcException {
        ...
        Matcher bracketsMatcher = Pattern.compile(Patterns.BKT).matcher(expression);
        while (bracketsMatcher.find()) {
            String group = bracketsMatcher.group();
            String resultExpressionResult = String.valueOf(calc(group.substring(1, group.length() - 1)));
            expression = bracketsMatcher.replaceFirst(resultExpressionResult);
            bracketsMatcher.reset();
            bracketsMatcher = Pattern.compile(Patterns.BKT).matcher(expression);
        }
